import re
from stock.models import StockProduct, StockOutReport, CasesPalu, Tests, PotentialCases, PotentialDeceased, Reporter
...
def create_stockproduct(report=None, product=None, *args, **kwargs):
    ...
    if re.match(r'^(RP)\s+(\d{6})\s+(qui|ACT|ART|TDR|SP)(\s+\d+)$', report.text.strip(), re.I):
    ...
    elif re.match(r'^(SF|SR|SD)\s+(\d{6})\s+(qui|ACT|ART|TDR|SP)(\s+\d+){1,4}$', report.text.strip().strip(), re.I):
        values = report.text.strip().strip().split(" ")[3:]
        if report.text.strip().strip().split(" ")[0] in ["SF"] and report.reporting_date.weekday() != 0:
...
def update_stockproduct(report=None, product=None, *args, **kwargs):
    ...
    elif re.match(r'^(SF|SR|SD)\s+(\d{6})\s+(qui|ACT|ART|TDR|SP)(\s+\d+){1,4}$', report.text.strip(), re.I):
        if report.text.strip().split(" ")[0] in ["SF"] and report.reporting_date.weekday() != 0:
            return "Itariki mwandiste siyo, kuko itariki bandika ari iyo ku wambere. Bibwire uwubatwara."
