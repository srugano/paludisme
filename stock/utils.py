...
class BookResource(resources.ModelResource):
    class Meta:
        ...
        fields = (
            ...
            "facility__district__province__name",
            "reporting_date",
            "text",
            "category",
        )
...
def export_to_excel(request, *args, **kwargs):
    ...
    if request.POST.get("startdate", ""):
        queriset = queriset.filter(
            reporting_date__gte=request.POST.get("startdate", "")
        )
    if request.POST.get("cds", ""):
        queriset = queriset.filter(
            facility__code=json.loads(request.POST.get("cds", ""))["code"]
        )
    if request.POST.get("district", ""):
        queriset = queriset.filter(
            facility__district__code=json.loads(request.POST.get("district", ""))[
                "code"
            ]
        )
    if request.POST.get("province", ""):
        queriset = queriset.filter(
            facility__district__province__code=json.loads(
