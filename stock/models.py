...
class CasesPalu(models.Model):
    ...
    def __unicode__(self):
        return "{0} - {1} - {2} - {3}".format(
            self.simple, self.acute, self.pregnant_women, self.decease
        )
...
class Temporary(models.Model):
    ...
    phone_number = models.CharField(
        ...
        validators=[phone_regex],
        blank=True,
        help_text=_("The telephone to contact the sender."),
        max_length=16,
    )
    supervisor_phone_number = models.CharField(
        _("telephone"),
        validators=[phone_regex],
        help_text=_("The telephone to contact your supervisor."),
        max_length=16,
        null=True,
        blank=True,
    )
...
class StockOutReport(models.Model):
    ...
    def __unicode__(self):
        return "{0} => Remaining quantity: {1}".format(
            self.product.designation, self.remaining
        )
